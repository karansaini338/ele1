    ...
    <section className="py-24 px-4 bg-white relative">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col items-center text-center mb-16">
          <span className="text-rajasthani-green font-medium tracking-wider text-sm uppercase mb-3">Explore</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-6">Book Your Elephant Experience in Jaipur Today</h2>
          <div className="w-24 h-1 bg-rajasthani-green rounded-full mb-6"></div>
          <p className="text-lg text-gray-600 max-w-2xl">
           Escape the ordinary and join us at Ele Experience Jaipur for a soulful, ethical adventure that celebrates elephants, conservation, and the rich culture of Rajasthan.
          </p>
        </div>
        ...
        {experiences.map((experience, index) => (
          <div
            key={index}
            className={`flex flex-col ${experience.imagePosition === "right" ? "md:flex-row-reverse" : "md:flex-row"} items-center mb-24 overflow-hidden rounded-2xl border border-green-100 bg-white`}
          >
              ...
              <div className="aspect-[4/3]  relative overflow-hidden">
                ...
                {experience.badge && (
                  <div className="absolute top-4 left-4">
                    <span className={`${experience.badge.color} text-white text-xs font-bold px-3 py-1.5 rounded-full`}>
            ...
            <div className="md:w-1/2 p-8 md:p-12">
              <h3 className="text-2xl font-bold mb-4 text-rajasthani-green">{experience.title}</h3>
              <p className="text-gray-600 mb-6">{experience.description}</p>
              <div className="flex flex-wrap gap-2">
                {experience.tags.map((tag, tagIndex) => (
                  <span key={tagIndex} className="bg-green-50 text-rajasthani-green text-xs px-3 py-1 rounded-full">
                    {tag}
                  </span>
                ))}
              </div>
